...
class GenericAPIModelConfig(PydanticBaseModel):
    ...
    use_litellm: str | None = "true" # (TODO)
    reasoning_effort: str | None = "medium" # (TODO)
...
class LiteLLMModel(AbstractModel):
    ...
    def _single_query(
        ...
        try:
            if ('gpt-5' in self.config.name.lower() or 'o3' in self.config.name.lower()) and (self.config.use_litellm == "true"):  # (TODO)
                response: litellm.types.utils.ModelResponse = litellm.completion(  # type: ignore
                    model=self.config.name,
                    messages=messages,
                    temperature=self.config.temperature if temperature is None else temperature,
                    top_p=self.config.top_p,
                    drop_params=True, # (TODO): handle for LiteLLM
