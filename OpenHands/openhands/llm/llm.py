...
class LLM(RetryMixin, DebugMixin):
    ...
    def __init__(
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            # print(f'[self.config.model] = {self.config.model}')
            if 'litellm_proxy' not in self.config.model:
                kwargs.pop('extra_body', None)
            else:
                if 'gpt-5' in self.config.model.lower() or 'o3' in self.config.model.lower():
            ...
            # print(f'[type messages] = {type(kwargs["messages"])}')
            with warnings.catch_warnings():
